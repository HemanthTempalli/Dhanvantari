...
def send_message():
    try:
        ...
        # Extract and store predicted disease if booking is shown (final recommendation)
        if result.get('show_booking', False):
...
def extract_predicted_disease(summary_text):
    """Extract predicted disease/condition from AI summary text"""
    if not summary_text:
        return None
    
    text_lower = summary_text.lower()
    
    # Common patterns for disease predictions
    patterns = [
        r"likely.*?([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)",
        r"probable.*?([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)",
        r"suggest.*?([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)",
        r"condition.*?([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)",
        r"diagnosis.*?([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)"
    ]
...
def extract_confidence_level(summary_text):
    """Extract confidence level from AI summary text"""
    if not summary_text:
        return 0.75
    ...
    if "high confidence" in text_lower or "highly confident" in text_lower:
        ...
        return 0.80
    elif "low confidence" in text_lower or "lower confidence" in text_lower:
        return 0.70
    elif "medium confidence" in text_lower:
        return 0.75
    
    # Look for other confidence indicators
    if any(phrase in text_lower for phrase in ["strongly suggest", "clearly indicate", "definitively"]):
        return 0.85
    elif any(phrase in text_lower for phrase in ["likely", "probable", "suggest"]):
        return 0.80
    elif any(phrase in text_lower for phrase in ["possible", "may indicate", "could be"]):
        return 0.70
...
def download_report():
    ...
    try:
        ...
        user_data = {
            ...
            'confidence_level': session.get('confidence_level', 0.75),
...
def book_appointment():
    ...
    # Extract and store confidence level
    confidence_level = extract_confidence_level(summary)
